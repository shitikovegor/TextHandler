...
public class TextReaderTest {
    ...
    @Test
    public void testReadFile() throws ProjectException {
        TextReader reader = TextReader.getInstance();
        String actual = reader.readFile("input_test/text.txt");
        String expected = "\tIt has survived - not only (five) centuries, but also the leap into 13+ i-- " +
                ...
                " (71-(2*i--*(3*(2-1/2*2)-2)-10/2))*++i Ipsum is that it has a more-or-less normal distribution" +
                " of letters, as opposed to using (Content here), content here', making it look like readable" +
                " English.\tIt is a (-5+1/2*(2+5*2- --j))*1200 established fact that a reader will be of a page" +
                " when looking at its layout.\tBye (5+7)x.";
